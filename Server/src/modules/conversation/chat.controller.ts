    ...
    try {
      ...
      const conversationsWithUserInfo = await Promise.all(
        conversations.map(async (conv) => {
          const partnerId = conv.userId1 === user_id ? conv.userId2 : conv.userId1;

          let partnerInfo = null;
          try {
            partnerInfo = await userService.getUserById(partnerId);
          } catch (error) {
            console.error(`Error fetching user ${partnerId}:`, error);
          }

          if (!partnerInfo) {
            // 2. Nếu không tìm thấy user -> Xóa cuộc trò chuyện và tin nhắn
            console.log(`User ${partnerId} not found, deleting conversation ${conv.conId}`);
            await Message.deleteMany({ conId: conv.conId });
            await Conversation.deleteOne({ conId: conv.conId });
            return null;
          }
          ...
          return {
            ...conv,
            partnerInfo: {
              userId: partnerInfo.userId || partnerId,
      ...
      // Lọc bỏ các cuộc trò chuyện đã bị xóa (null)
      const validConversations = conversationsWithUserInfo.filter((c) => c !== null);
  ...
  static async sendMessage(req: Request, res: Response) {
    upload(req, res, async (err: any) => {
      ...
      try {
        ...
        // 3. Kiểm tra xem receiver có tồn tại không
        const receiverExists = await userService.getUserById(receiver_id);
        if (!receiverExists) {
