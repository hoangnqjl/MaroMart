...
class ProductRepository implements IProductRepository {
    ...
    async getAllProducts(page: number = 1, limit: number = 3) {
    ...
    async getProductById(productId: string): Promise<IProduct | null> {
        return await Product.findOne({ productId }).exec();
    }
    ...
    async searchProductByName(keyword: string): Promise<IProduct[]> {
        ...
        // Tách từ khóa thành nhiều phần
        const tokens = keyword.trim().split(/\s+/); // VD: "An Khê" → ["An", "Khê"]
        ...
        // Dùng $and để đảm bảo tất cả từ đều phải match
