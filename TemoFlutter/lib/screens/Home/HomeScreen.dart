...
class HomeScreenState extends State<HomeScreen> {
  ...
  final List<String> _searchHints = ["Tìm kiếm sản phẩm..."];
  ...
  Future<void> _loadCategories() async {
    ...
    try {
      ...
      cats.sort((a, b) {
        final String idA = (a['categoryId'] ?? a['id'] ?? '')
            .toString()
            .toLowerCase();
        final String idB = (b['categoryId'] ?? b['id'] ?? '')
            .toString()
            .toLowerCase();
  ...
  Future<void> _loadRecommendedProducts() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          // Lọc bỏ sản phẩm của chính người dùng hiện tại (so sánh an toàn)
          final filteredItems = (result['items'] as List<Product>? ?? []).where(
            (p) {
              final prodUserId = p.userId.trim().toLowerCase();
              final currentId = _currentUserId?.trim().toLowerCase();
              return prodUserId != currentId;
            },
          ).toList();

          _recommendedProducts = filteredItems;
  ...
  Widget _buildCategoryItem(dynamic cat) {
    final String id =
        cat['categoryId']?.toString() ?? cat['id']?.toString() ?? '';
    final String name =
        cat['categoryName']?.toString() ?? cat['name']?.toString() ?? '';
    final String? iconName = cat['categoryIcon']
        ?.toString(); // URL from backend
      ...
      if (mounted) {
        setState(() {
          if (newProducts.isEmpty) {
            ...
            // VÀ lọc bỏ sản phẩm của chính mình
            final existingIds = _products.map((p) => p.productId).toSet();
            final uniqueNewProducts = newProducts.where((p) {
              final isDuplicate = existingIds.contains(p.productId);
              return !isDuplicate;
            }).toList();
            ...
            if (!isFiltering && newProducts.length < _limit) {
              _hasMore = false;
            }
          }
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: AnimatedContainer(
                            ...
                            child: Container(
                              margin: _searchFocusNode.hasFocus
                            ...
                            : ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  if (imageUrl.startsWith('image:'))
                                    imageUrl = imageUrl.substring(6).trim();
                                                        ...
                                                        onTap: () {
                                                          if (product
                                                            ...
                                                            final partner = ChatPartner(
                                                              ...
                                                              fullName: userInfo
                                                                  .fullName,
                                                              avatarUrl: StringUtils.normalizeUrl(
                                                                product
                                                                        .userInfo
                                                                        ?.avatarUrl ??
                                                                    '',
            ...
            // 4. AI ASSISTANT FLOATING BUTTON
            Positioned(
              bottom: 110,
              right: 15,
              child: GestureDetector(
                    ...
                    MaterialPageRoute(
                      builder: (context) => ChatScreen(
                        ...
                        partnerUser: ChatPartner(
                          userId: "ai_assistant",
                          fullName: "Trợ lý Temo",
                          avatarUrl: "assets/images/ai_nv.png",
                        ),
                      ),
                    ),
                  );
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: const Color(0xFFFFB86B).withOpacity(0.5),
                      width: 1.5,
                    ),
                  ),
                  child: ClipOval(
